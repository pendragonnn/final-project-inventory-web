...
const DetailReportReturning = () => {
	...
	const handleExportPDF = () => {
		...
		doc.setFontSize(16);
		const text = "REPORT RETURNING DETAIL";
		...
		let outletOrSupplier = "-";
		if (data.Outlet) {
			// If Outlet exists
			outletOrSupplier = `Outlet: ${data.Outlet.name}`;
		} else if (data.Supplier) {
			// If Supplier exists
			outletOrSupplier = `Supplier: ${data.Supplier.name}`;
		}
		...
		doc.text(outletOrSupplier, margin, yPosition + 8);
		...
		data.TransactionDetails.forEach((item, index) => {
			...
			doc.setFontSize(10);
			doc.text(`Reason: ${item.reason}`, margin, yPosition + 6);
			doc.setFont("helvetica", "bold");
			doc.setFontSize(12);
		...
		doc.save("report_returning_detail.pdf");
	};
						...
						<div className="bg-white dark:bg-boxdark rounded-lg shadow-md">
							<div className="flex flex-row items-center justify-between p-6">
								<h1 className="text-2xl font-bold text-gray-900 dark:text-white">
									Report Returning Detail :&nbsp;{data.id}
								</h1>
...
export default DetailReportReturning;
