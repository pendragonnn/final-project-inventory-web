...
const Dashboard = () => {
	...
	useEffect(() => {
		...
		if (!token) {
			console.log("Token not found, redirecting to forbidden");
		...
		try {
			...
			console.log("Decoded Token:", decoded); // Debugging: Periksa klaim token

			if (!decoded.role) {
				console.log("Role not found in token, redirecting to forbidden");
				router.push("/forbidden");
				return;
			}
