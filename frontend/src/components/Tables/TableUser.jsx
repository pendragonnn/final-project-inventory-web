...
import ModalImageUser from "../Modal/User/ModalImageUser";
...
const TableSupplier = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				const res = await User.getUsers(currentPage, size);
	...
	const handleEditData = async (updateUser) => {
		setData((prevData) =>
			prevData.map((user) => (user.id === updateUser?.id ? updateUser : user))
		);
							...
							{isFilterMenuOpen && (
								<div className="absolute right-0 mt-2 w-48 bg-white dark:bg-meta-4 dark:text-white border border-gray-200 rounded-md shadow-lg">
									{[
										...
										{ field: "full_name", order: "asc", label: "Name Asc" },
										{ field: "full_name", order: "desc", label: "Name Desc" },
										{ field: "Role.name", order: "asc", label: "Role Asc" },
										{ field: "Role.name", order: "desc", label: "Role Desc" },
									].map((option) => (
										<button
						...
						{isDataEmpty ? (
							...
							paginatedData.map((value, key) => (
								...
								>
									...
									<td className="border-b border-[#eee] py-2 px-4 dark:border-strokedark flex items-center justify-center">
										<div>
											<img
												src={`/uploads/user/${value.image_url}`}
												className="w-10 h-10 rounded-full cursor-pointer object-cover"
												onClick={() => handlefoto(value.id, value.image_url)}
											/>
										</div>
									</td>
									<td className="border-b border-[#eee] px-4 text-center dark:border-strokedark mx-auto">
										...
										<p
											className={
												value.Role.name === "Admin"
													? "text-danger" // Admin warna merah
													: value.Role.name === "Owner"
													? "text-meta-3" // Manager warna biru
													: "text-primary" // Staff warna hitam
											}
										>
											{value.Role.name}
										</p>
									...
									<td className="border-b border-[#eee] px-4 text-center dark:border-strokedark mx-auto">
										<p className="dark:text-white text-black">{value.email}</p>
									</td>
									<td className="border-b border-[#eee] px-4 text-center dark:border-strokedark">
										<div className="flex justif-center space-x-3.5">
											<label
												htmlFor="edit"
